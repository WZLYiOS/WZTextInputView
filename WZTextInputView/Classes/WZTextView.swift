...
/// MARK - WZTextViewDelegate
@objc public protocol WZTextViewDelegate: UITextViewDelegate {
    
    /**
     *  输入框高度发生变化时的回调，仅当 `autoResizable` 属性为 YES 时才有效。
     *  @note 只有当内容高度与当前输入框的高度不一致时才会调用到这里，所以无需在内部做高度是否变化的判断。
     */
    @objc optional func textView(_ textView: WZTextView,
     ...
     */
    @objc optional func textViewShouldReturn(_ textView: WZTextView) -> Bool
...
/// MARK - WZTextView
@objcMembers
open class WZTextView: UITextView {
    ...
    /// 原始委托
    private weak var originalDelegate: WZTextViewDelegate?
    ...
    open override var delegate: UITextViewDelegate? {
        ...
        set {
            if newValue as? NSObject != self {
                originalDelegate = newValue as? WZTextViewDelegate
            } else {
...
extension WZTextView {
    ...
    private func handleTextChanged(_ sender: AnyObject) {
        ...
        var textView: WZTextView?
        
        if sender is Notification {
            let object = (sender as! Notification).object
            if object is WZTextView {
                textView = object as? WZTextView
            }
        } else if sender is WZTextView {
            textView = sender as? WZTextView
        }
...
extension WZTextView: UITextViewDelegate {
